...
#                                                                             
# PROGRAMMER: APARNA B.S
# DATE CREATED: 28th Oct 2018                                  
# REVISED DATE: 
...
def get_pet_labels(image_dir):
    ...
    """
    # Retrieve filenames from folder pet_image/
    in_files = listdir('pet_images')
    
    # Print 10 of the filenames from folder pet_images/
    print("\nPrints 10 filenames from folder pet_images/")
    for idx in range(1, 11):
        print("{:2d} file: {:>25}".format(idx, in_files[idx]) )
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           low_word_list_pet_image = in_files[idx].lower().split("_")
            
           # Loops to check if word in pet name is only
           # alphabetic characters - if true append word
           # to pet_name separated by trailing space 
           for word in low_word_list_pet_image:
               if word.isalpha():
                   pet_label += word + " "
